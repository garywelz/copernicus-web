...
# Tier-1 write pre-flight (2026-09-19)

1. **`--cited-by` uses `"TDAP seed set"`, not Jordan's name.** Jordan hasn't yet been asked whether his name should appear in public Firestore records (separate from the already-open README-name question). **Field to revisit once he answers**: `citations[].cited_by` on the six seed-intake records and on any `would_merge` citation events this write creates.
2. **Title-cleaning fix applied before writing, per the "one-line, no ID impact" condition.** Checked empirically against all 124 real records (not reasoned about) before touching code: re-resolved every one via the exact functions the write path uses, computed `doc_id` before and after a proposed `_clean_title()` (strips HTML tags, collapses whitespace), and diffed. Result: **2 of 124 titles actually needed cleaning** (`10.21105/joss.05791` — an embedded `\n`, exactly the record Chat named; `10.2139/ssrn.2903278` — a double space, found by the same check, not previously flagged), and **0 of 124 doc_ids changed** — every one of these records resolves via Crossref/bioRxiv to a DOI-derived `crossref_<doi>`-style id, never the title-hash fallback path. Condition met; added `_clean_title()` to `citation_expansion_pilot.py` and applied it to `record["title"]` right after resolution, before the title-mismatch check and before `doc_id`/write. `py_compile` clean.
3. **Dry run re-verification, immediately before writing**: re-ran the exact v2 command. Identical result: 124 admitted, 123 would_create, 1 would_merge, 0 title_mismatch, 0 unresolved — and the **124-DOI admitted set is byte-for-byte identical** to the earlier v2 run (checked directly, not inferred). All three pre-flight conditions satisfied.

## Stopped mid-execution — an unplanned second step in seed intake

Ran seed-intake call 1 of 6 (de Silva–Morozov–Vejdemo-Johansson). **`researcher_cited_intake.py --write` does not reach Firestore.** Its own console output says so directly: it writes a local metadata JSON file (`huggingface-space/metadata-database/papers/mathematics/researcher_cited_crossref_10.1007_s00454-011-9344-x.json`) and states *"This is a metadata JSON file only — it still needs `cloud-run-backend/scripts/ingest_papers_from_metadata_json.py` to reach Firestore."* Confirmed via a live, read-only Firestore query immediately after: zero docs with `cited_project == "tdap"`, zero docs matching that DOI — **no Firestore mutation occurred.** Stopped here rather than running the remaining 5 seed-intake calls or guessing at the missing step, per the explicit instruction to stop and report on anything unexpected.

**What running the second script would actually do, checked before proposing anything**: `ingest_papers_from_metadata_json.py`'s default `--root` is the *entire* local metadata mirror (`huggingface-space/metadata-database/papers/`), which currently holds 12,235 files, not just the 6 new ones — its own log line confirmed this count. Its default `--include-glob` is `**/*.json`, so a bare invocation would scan every file in that tree. Checked `--skip-existing` (default `True`): it would prevent re-writing anything already in Firestore, and a live check confirmed the 3 pre-existing `researcher_cited_*.json` files already sitting in that directory from past sessions (unrelated to TDAP: `10.1007/bfb0080769`, `10.1090/s0002-9947-1953-0053041-6`, `10.2307/2371045`) are indeed already in Firestore — so a broad scan wouldn't overwrite them. But scanning 12,235 files to reach 6 is far more than "writes limited to the 130 records described," and I don't want to rely on a safety-net default when a precisely-scoped alternative exists.

**Proposed fix, not yet applied**: move the 6 new `researcher_cited_crossref_*.json` files into an isolated temp directory (e.g. `papers/_tdap_seed_intake_2026-09-19/`) and run `ingest_papers_from_metadata_json.py --root papers/_tdap_seed_intake_2026-09-19 --dry-run` first, then without `--dry-run`, with `--no-reject-gcs` added (no rejects expected, and avoiding any GCS interaction not already accounted for in "writes limited to the 130 records"). This scopes the ingest scan to exactly the 6 seed files, not the surrounding 12,235.

## Approved (isolated-directory approach) — seed intake and scoped ingest, executed

Ran the remaining 5 seed-intake calls. **One more unexpected wrinkle, handled inline (low-risk, no Firestore/GCS touched, reported here rather than silently)**: Scoccola et al.'s DOI (`10.4230/lipics.socg.2023.57`) isn't on Crossref at all (confirmed earlier this session) — `researcher_cited_intake.py --input <doi>` failed cleanly (queued to a local review file, zero mutation) rather than writing anything wrong. Retried with the arXiv id instead (`--input 2212.07201`, no `--id-type` override needed — `classify()`'s bare-arXiv regex matched it directly) — succeeded. **Limit**: the resulting seed record has `doi: null` (arXiv metadata for this entry doesn't cross-reference the published DOI); its Firestore doc id is `arxiv_2212.07201v2`, not DOI-derived. Also noticed in passing: Perea's seed record has an unescaped LaTeX artifact in its title (`"Sparse Circular Coordinates via Principal $$\mathbb {Z}$$-Bundles"`) — a different code path than the `_clean_title()` fix above (seed intake via `researcher_cited_intake.py` never calls it), so not caught by that fix. Flagging both as limits, neither blocking.

Copied all 6 resulting JSON files into `papers/_tdap_seed_intake_2026-09-19/` (isolated from the 12,235-file mirror). `--dry-run` confirmed "Would write: 6, Skipped: 0, Failed: 0" scoped to exactly this directory. Ran for real with `--no-reject-gcs`: **Wrote: 6, Skipped: 0, Failed: 0** — the first real Firestore write of this project. Verified immediately, read-only: exactly 6 docs with `cited_project == "tdap"`, correct titles, correct `question_scope_ids` (3× `tdap-q1`, 3× `tdap-q2`), 5 of 6 with `doi` populated (Scoccola's is `null`, as expected from the limit above).

## Step 5 (pilot `--write`) — incomplete, stopped rather than worked around

Ran the approved command exactly as specified (six-seed file, `--only-dois-file` restricted to the 124-DOI allowlist, no `--enrich-cited-by-count`). Result: **`admitted: 126` → filtered to 109 by `--only-dois-file` → 108 created, 1 merged, 0 unresolved, 0 title_mismatch.** Every dry run (including the one run immediately before this write) had confirmed 124, not 109 — a real, unexplained-until-checked shortfall of 15.

**Diagnosed precisely, not guessed**: compared this run's per-seed fetch log against the immediately-prior dry run's. Nigmetov–Morozov had 0 references in **both** runs (`status: error`, persistent 429 — consistent, not a new regression). **Scoccola et al. flipped**: 29 references via Semantic Scholar in the dry run, but a 429 (exhausting all 3 retries) during this exact write. All 15 missing DOIs have Scoccola as a parent (14 with Scoccola as sole parent via `all_references_from_seed`/`top_cited_in_seed`, 1 via `cited_by_2plus_seeds` where Scoccola was one of three parents). **Nothing wrong was written** — `--only-dois-file` correctly restricted output to the approved list throughout; the run is simply incomplete, not incorrect.

**Firestore reconciliation, read-only**: `cited_project == "tdap"` (flat field) returns 114 = 6 seeds + 108 created. The 1 merged doc (`10.1038/nbt.3854`, the GLMP paper) doesn't show up in that flat-field count by design — merging only touches `question_scope_ids`/`citations[]`, never the flat `cited_project` field on an existing doc (exactly the reconciliation note Chat's own verification step anticipated). 114 + 1 merged + 15 missing = 130, the full target.

**Not yet done**: 15 approved candidates unwritten; embedding backfill not started (total isn't ~130 yet); no further verification steps run. Stopped here to report and propose rather than silently retrying, per instructions. **Proposed**: retry the identical pilot `--write` command as-is — the existing 109 records are dedup-safe (checked by doc_id/doi first, would show as `already_in_corpus`/merge, not duplicate), so a full re-run is the same idempotent operation step 8 was already going to test, just run one step earlier than planned. If Scoccola's S2 call succeeds this time (it did once already), this should close the gap to 124/130.

## Retry — gap closed, tier-1 write complete (2026-09-19)

Retried the identical command, approved. This time **both** S2-dependent seeds succeeded (Scoccola 29 refs, **Nigmetov–Morozov 21 refs — the first time all session it returned anything**): 124 admitted, 15 created, 109 correctly detected as `already_in_corpus` and safely merged (no duplicates). Verified directly: **all 124 approved DOIs now written, zero missing.** Firestore: `cited_project == "tdap"` (flat) = 129 (6 seeds + 108 + 15), + 1 merge-only doc = **130, the full target.**

## Steps 7-11 — verification and live checks

**Step 7 (Firestore, read-only)**: `question_scope_ids` values across all 129 flat-tagged docs are **exclusively** `tdap-q1` (35) and `tdap-q2` (99) — no stray tags. The merged doc (`10.1038/nbt.3854`) carries `question_scope_ids: ['glmp-q9', 'tdap-q2']` — both intact, exactly as required — with `cited_project` (flat) still `None` (unchanged by design) and a new `citations[]` entry (`cited_project: tdap`, `cited_by: citation_expansion_pilot`).

**Step 8 (idempotency)**: two attempts hit a transient Firestore `503`/connection-reset (infrastructure hiccup, not a logic bug — occurred during a pure read-only dry run, zero risk). Third attempt succeeded: **0 `would_create`, 100% `would_merge`** for every candidate found that run — the property being tested (no duplicates for already-written records) holds regardless of S2's per-run flakiness in how many candidates it re-discovers.

**Step 9 (embedding coverage)**: `--pin` census found exactly **129** unembedded docs (matches the 129 flat-tagged records; the 130th — the merged GLMP paper — was already embedded pre-TDAP and correctly didn't need re-embedding). `--run` completed **129/129, 0 failed, 0 skipped**. Total TDAP-related embedding coverage: **130 of 130.**

**Step 10 (live checks, plain fetches)**:
- Regenerated and republished `knowledge-engine-status.json`: `papers_by_initiative: {glmp: 45456, atap: 2822, tdap: 130}`. Confirmed on the live production Statistics tab via Playwright (`SHOWS_TDAP_130: true`) — no redeploy needed, this stat reads the GCS JSON at runtime.
- Real query on the live Knowledge Engine (`/api/rag/answer`, "circular coordinates from persistent cohomology"): the answer is now substantively correct and on-topic (describes the de Silva–Morozov–Vejdemo-Johansson algorithm accurately) — a stark contrast to the same class of query tested before any TDAP papers existed. **7 of the top 10 cited results are genuine TDAP records**, cross-checked directly against Firestore `question_scope_ids`/`cited_project` (not the RAG API's own citation objects, which show `cited_project: None` for every citation regardless of project — confirmed as an API-response display quirk, not a data problem: the same 7 records show `cited_project: tdap` when queried directly). The other 3 of 10 are pre-existing ATAP/general-math corpus content matched by loose "cohomology" keyword overlap — the same known unscoped-search limitation from earlier in this doc, now diluted rather than dominant because real on-topic content exists.
