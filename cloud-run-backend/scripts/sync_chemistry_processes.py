...
CHEMISTRY_PROCESSES_BUCKET_PATH = "chemistry-processes-database"


def create_text_for_chemistry_process(process_data: Dict[str, Any]) -> str:
...
def convert_chemistry_to_firestore_format(process_data: Dict[str, Any], process_id: str, file_path: str) -> Dict[str, Any]:
    """Convert chemistry process data to Firestore format."""
    return {
        "process_id": process_id,
        "title": process_data.get('title', process_id),
        "description": process_data.get('description', ''),
        "category": process_data.get('category', 'chemistry'),
...
def list_chemistry_process_files(bucket_name: str, bucket_path: str) -> List[str]:
    """List all chemistry process JSON files in GCS."""
...
def get_chemistry_process_file(bucket_name: str, file_path: str) -> Optional[Dict[str, Any]]:
    """Get chemistry process JSON file from GCS."""
    try:
        client = storage.Client()
        bucket = client.bucket(bucket_name)
        ...
        logger.error(f"Failed to load chemistry process from {file_path}: {e}")
        return None


def sync_chemistry_processes(
    dry_run: bool = False,
    limit: Optional[int] = None,
    skip_existing: bool = True
) -> Dict[str, Any]:
    """
    Sync chemistry processes from GCS to Firestore.
    ...
    try:
        # Get all chemistry process files from GCS
        print("📋 Fetching chemistry processes from Google Cloud Storage...")
        all_files = list_chemistry_process_files(GCS_BUCKET_NAME, CHEMISTRY_PROCESSES_BUCKET_PATH)
        ...
        print(f"\n📊 Found {stats['total_in_gcs']} chemistry processes in GCS")
        print(f"   Dry run: {dry_run}")
        print(f"   Limit: {limit or 'None (all)'}")
        print(f"   Skip existing: {skip_existing}\n")
        
        # Check existing processes in Firestore
        firestore_chemistry_ref = firestore_db.collection('chemistry_processes')
        existing_process_ids = set()
        
        if skip_existing:
            print("🔍 Checking existing processes in Firestore...")
            for doc in firestore_chemistry_ref.stream():
        ...
        for i, file_path in enumerate(all_files, 1):
            try:
                ...
                process_id = file_path.replace(CHEMISTRY_PROCESSES_BUCKET_PATH + "/", "").replace(".json", "")
                ...
                process_data = get_chemistry_process_file(GCS_BUCKET_NAME, file_path)
                if not process_data:
                    stats["failed"] += 1
                ...
                chemistry_doc = convert_chemistry_to_firestore_format(process_data, process_id, file_path)
                
                # Generate embedding
                has_embedding = False
                if embedding_available and embedding_service:
                    try:
                        text = create_text_for_chemistry_process(process_data)
                        if text:
                            embedding = embedding_service.embed_text(text)
                            if embedding:
                                chemistry_doc["embedding"] = Vector(embedding)
                                chemistry_doc["embedding_model"] = resolve_embedding_model_name(embedding_service)
                                chemistry_doc["embedding_updated"] = datetime.utcnow().isoformat()
                                has_embedding = True
                ...
                # Write to Firestore
                if not dry_run:
                    firestore_chemistry_ref.document(process_id).set(chemistry_doc)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Sync chemistry processes from GCS to Firestore")
    ...
    print("="*70)
    print("  SYNC CHEMISTRY PROCESSES TO FIRESTORE")
    print("="*70)
    print()
    
    stats = sync_chemistry_processes(
