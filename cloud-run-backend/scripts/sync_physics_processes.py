...
PHYSICS_PROCESSES_BUCKET_PATH = "physics-processes-database"


def create_text_for_physics_process(process_data: Dict[str, Any]) -> str:
...
def convert_physics_to_firestore_format(process_data: Dict[str, Any], process_id: str, file_path: str) -> Dict[str, Any]:
    """Convert physics process data to Firestore format."""
    return {
        "process_id": process_id,
        "title": process_data.get('title', process_id),
        "description": process_data.get('description', ''),
        "category": process_data.get('category', 'physics'),
...
def list_physics_process_files(bucket_name: str, bucket_path: str) -> List[str]:
    """List all physics process JSON files in GCS."""
...
def get_physics_process_file(bucket_name: str, file_path: str) -> Optional[Dict[str, Any]]:
    """Get physics process JSON file from GCS."""
    try:
        client = storage.Client()
        bucket = client.bucket(bucket_name)
        ...
        logger.error(f"Failed to load physics process from {file_path}: {e}")
        return None


def sync_physics_processes(
    dry_run: bool = False,
    limit: Optional[int] = None,
    skip_existing: bool = True
) -> Dict[str, Any]:
    """
    Sync physics processes from GCS to Firestore.
    ...
    try:
        # Get all physics process files from GCS
        print("📋 Fetching physics processes from Google Cloud Storage...")
        all_files = list_physics_process_files(GCS_BUCKET_NAME, PHYSICS_PROCESSES_BUCKET_PATH)
        ...
        print(f"\n📊 Found {stats['total_in_gcs']} physics processes in GCS")
        print(f"   Dry run: {dry_run}")
        print(f"   Limit: {limit or 'None (all)'}")
        print(f"   Skip existing: {skip_existing}\n")
        
        # Check existing processes in Firestore
        firestore_physics_ref = firestore_db.collection('physics_processes')
        existing_process_ids = set()
        
        if skip_existing:
            print("🔍 Checking existing processes in Firestore...")
            for doc in firestore_physics_ref.stream():
        ...
        for i, file_path in enumerate(all_files, 1):
            try:
                ...
                process_id = file_path.replace(PHYSICS_PROCESSES_BUCKET_PATH + "/", "").replace(".json", "")
                ...
                process_data = get_physics_process_file(GCS_BUCKET_NAME, file_path)
                if not process_data:
                    stats["failed"] += 1
                ...
                physics_doc = convert_physics_to_firestore_format(process_data, process_id, file_path)
                
                # Generate embedding
                has_embedding = False
                if embedding_available and embedding_service:
                    try:
                        text = create_text_for_physics_process(process_data)
                        if text:
                ...
                # Write to Firestore
                if not dry_run:
                    firestore_physics_ref.document(process_id).set(physics_doc)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Sync physics processes from GCS to Firestore")
    ...
    print("="*70)
    print("  SYNC PHYSICS PROCESSES TO FIRESTORE")
    print("="*70)
    print()
    
    stats = sync_physics_processes(
