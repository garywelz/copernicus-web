...
async def verify_admin_api_key(
    ...
    if not expected_key:
        print("⚠️  ADMIN_API_KEY not configured - admin endpoints will be inaccessible")
    ...
    if provided_key.strip() != expected_key.strip():
        print(f"❌ Invalid admin API key attempt")
    ...
    try:
        ...
        if include_episodes:
            try:
                ...
                print(f"⚠️  Could not fetch episodes collection: {e}")
        ...
        print(f"✅ Admin: Found {len(podcast_list)} podcasts for subscriber {subscriber_id} (from podcast_jobs and episodes)")
...
async def admin_delete_podcast(podcast_id: str, admin_auth: bool = Depends(verify_admin_api_key)):
    ...
    try:
        ...
        if is_in_rss:
            ...
            try:
                await _update_rss_feed(podcast_data, subscriber_payload, False, None)
                print(f"✅ Admin: Removed podcast {podcast_id} from RSS before deletion")
            except Exception as rss_error:
                print(f"⚠️  Warning: Failed to remove podcast from RSS before deletion: {rss_error}")
        ...
        if canonical:
            try:
                ...
                print(f"✅ Admin: Deleted episode catalog entry: {canonical}")
            except Exception as e:
                print(f"⚠️  Warning: Failed to delete episode catalog entry: {e}")
        ...
        print(f"✅ Admin: Deleted podcast {podcast_id}")
        ...
        print(f"❌ Error deleting podcast (admin): {e}")
        import traceback
        traceback.print_exc()
