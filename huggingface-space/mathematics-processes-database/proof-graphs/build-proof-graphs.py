...
STYLE = r"""    :root {
    ...
    * { box-sizing: border-box; }
    ...
    .legend-source { background: #e0f2fe; color: #0369a1; }
    .legend-assumption { background: #fef3c7; color: #b45309; }
    .legend-construction { background: #dcfce7; color: #15803d; }
    .legend-assertion { background: #ede9fe; color: #7c3aed; }
    .legend-inference { background: #fce7f3; color: #be185d; }
    .legend-algorithm { background: #e0e7ff; color: #4338ca; }
    .legend-contradiction { background: #fee2e2; color: #dc2626; }
    .legend-conclusion { background: #ccfbf1; color: #0f766e; }
    .source-note {
      margin-top: 32px;
      color: var(--muted);
      font-size: 0.95rem;
...
MERMAID_SCRIPT = r"""  <script type="module">
    ...
    const roleStyles = `
classDef proofSource fill:#e0f2fe,stroke:#0369a1,color:#0f172a,stroke-width:2px;
classDef proofAssumption fill:#fef3c7,stroke:#b45309,color:#0f172a,stroke-width:2px;
classDef proofConstruction fill:#dcfce7,stroke:#15803d,color:#0f172a,stroke-width:2px;
classDef proofAssertion fill:#ede9fe,stroke:#7c3aed,color:#0f172a,stroke-width:2px;
classDef proofInference fill:#fce7f3,stroke:#be185d,color:#0f172a,stroke-width:2px;
classDef proofAlgorithm fill:#e0e7ff,stroke:#4338ca,color:#0f172a,stroke-width:2px;
classDef proofContradiction fill:#fee2e2,stroke:#dc2626,color:#0f172a,stroke-width:3px;
classDef proofConclusion fill:#ccfbf1,stroke:#0f766e,color:#0f172a,stroke-width:3px;
...
`;
    ...
    const roleRules = [
      ...
      ['proofInference', /^(Inference:|Branch:)/i],
    ...
    function addRoleClasses(source) {
      ...
      while ((match = nodePattern.exec(source)) !== null) {
        const [, nodeId, shape, label] = match;
...
LEGEND = """      <section class="proof-legend" aria-label="Proof graph color legend">
        ...
        <div class="proof-legend-grid">
          <div class="proof-legend-item"><span class="proof-legend-swatch legend-source"></span><span>Source / given / definition / lemma</span></div>
          <div class="proof-legend-item"><span class="proof-legend-swatch legend-assumption"></span><span>Assumption or temporary hypothesis</span></div>
          <div class="proof-legend-item"><span class="proof-legend-swatch legend-construction"></span><span>Construction or introduced object</span></div>
          <div class="proof-legend-item"><span class="proof-legend-swatch legend-assertion"></span><span>Assertion established in the proof</span></div>
          <div class="proof-legend-item"><span class="proof-legend-swatch legend-inference"></span><span>Inference, branch, or proof move</span></div>
          <div class="proof-legend-item"><span class="proof-legend-swatch legend-algorithm"></span><span>Algorithm or procedural step</span></div>
          <div class="proof-legend-item"><span class="proof-legend-swatch legend-contradiction"></span><span>Contradiction</span></div>
          <div class="proof-legend-item"><span class="proof-legend-swatch legend-conclusion"></span><span>Discharge or final conclusion</span></div>
        </div>
      </section>
"""
