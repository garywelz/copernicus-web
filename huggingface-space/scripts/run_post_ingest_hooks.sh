...
#   1. (future) embed gap backfill --auto   ← slot here, before publish
#   2. status publish
...
# Future embed stage (enable only after Jetson fastapi + schedule clearance):
# run_hook "embed_auto" "/media/sdcard/logs/embed_auto_cron.log" \
#   env ... python .../backfill_research_paper_embeddings.py --auto
