...
Parses Mermaid syntax to extract nodes and edges.
...
4. Ensure each node has a unique identifier in Mermaid syntax
"""
...
def parse_mermaid(mermaid_text: str) -> Tuple[Dict[str, str], List[Tuple[str, str]]]:
    ...
    Returns:
        - nodes: Dict mapping node_id to node_label
    ...
    # Extract node definitions: ID[Label] or ID(Label) or ID((Label))
    node_pattern = r'(\w+)\s*[\[(]([^\])]+)[\])]'
    ...
    # Extract edges: ID1 --> ID2 or ID1 -->|label| ID2
...
def analyze_nodes(nodes: Dict[str, str], edges: List[Tuple[str, str]]) -> Tuple[Dict, List[str]]:
    """
    ...
    # Check for duplicate IDs (shouldn't happen in Mermaid, but check anyway)
    if len(nodes) != len(set(nodes.keys())):
        issues.append("Duplicate node IDs found in Mermaid syntax")
    ...
    # Build node stats
    node_stats = {}
    all_node_ids = set(nodes.keys()) | set(incoming_edges.keys()) | set(outgoing_edges.keys())
    ...
    return node_stats, issues

def classify_and_count_nodes(node_stats: Dict, nodes: Dict[str, str]) -> Tuple[int, int, List[str]]:
    """
    ...
    Returns:
        ...
        - problematic: List of AND/OR node IDs with details
    """
    and_count = 0
    or_count = 0
    problematic = []
...
def ensure_unique_ids(mermaid_text: str) -> Tuple[str, List[str]]:
    """
    Ensure all nodes have unique identifiers in Mermaid syntax.
    
    Note: Mermaid syntax inherently uses unique IDs, but we check for consistency.
    Returns updated Mermaid text and list of changes.
    """
    changes = []
    ...
    if len(node_ids) != len(set(node_ids)):
        changes.append("WARNING: Duplicate node IDs found in Mermaid syntax")
    
    # For now, Mermaid syntax already enforces unique IDs
    # This function mainly validates
    return mermaid_text, changes

def update_complexity(data: Dict, and_count: int, or_count: int) -> Dict:
    """
...
def process_file(file_path: Path, dry_run: bool = False) -> Dict:
    ...
    try:
        ...
        if not mermaid_text:
            result['errors'].append("No 'mermaid' field found")
            return result
        
        # Parse Mermaid
        nodes, edges = parse_mermaid(mermaid_text)
        if not nodes:
            result['warnings'].append("No nodes found in Mermaid syntax")
            return result
        
        # Analyze nodes
        node_stats, issues = analyze_nodes(nodes, edges)
        result['warnings'].extend(issues)
        
        # Classify AND/OR nodes
        and_count, or_count, problematic = classify_and_count_nodes(node_stats, nodes)
        ...
        # Check unique IDs (Mermaid syntax should already enforce this)
        updated_mermaid, id_changes = ensure_unique_ids(mermaid_text)
        result['id_changes'] = id_changes
        
        # Update complexity if counts changed
        if and_count != current_and or or_count != current_or:
            if not dry_run:
                data = update_complexity(data, and_count, or_count)
...
def main():
        ...
        for json_file in json_files:
            ...
            if args.verbose or result['errors'] or result['problematic_nodes'] or result['fixes']:
                ...
                if result['problematic_nodes']:
                    print(f"  ⚠️  {len(result['problematic_nodes'])} AND/OR nodes (2+ in, 2+ out):")
                    for node in result['problematic_nodes'][:5]:  # Show first 5
                        print(f"    - {node}")
                if result['fixes']:
                    for fix in result['fixes']:
                        print(f"  ✅ {fix}")
                if result['id_changes']:
                    for change in result['id_changes']:
                        print(f"  ℹ️  {change}")
    ...
    if files_with_problematic > 0:
        ...
        print("   These need manual review to classify as either AND or OR")
